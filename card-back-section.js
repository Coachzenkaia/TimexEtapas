// card-back-section.js (VERSIÓN CON PUNTOS DE CONTROL)

// ... (la función formatDuration se queda igual) ...
...
window.TrelloPowerUp.iframe(null, {
    render: function(t, options) {
        console.log("PUNTO 1: La función render se ha ejecutado."); // <-- PUNTO DE CONTROL 1

        const contentEl = document.getElementById('content');

        return t.get('card', 'private', 'actions')
            .then(function(actions) {
                console.log("PUNTO 2: Se obtuvieron las acciones de la tarjeta.", actions); // <-- PUNTO DE CONTROL 2

                const cardMovements = [];
                const creationAction = actions.find(a => a.type === 'createCard');

                if (!creationAction) {
                    console.error("ERROR: No se encontró la acción de creación."); // <-- PUNTO DE CONTROL DE ERROR
                    contentEl.innerHTML = '<p>No se pudo determinar la fecha de creación.</p>';
                    return t.sizeTo(contentEl);
                }
                console.log("PUNTO 3: Acción de creación encontrada.", creationAction); // <-- PUNTO DE CONTROL 3
                ...
                console.log("PUNTO 4: Movimientos encontrados.", moveActions); // <-- PUNTO DE CONTROL 4
                ...
                console.log("PUNTO 5: Cálculos completados.", cardMovements); // <-- PUNTO DE CONTROL 5
